...
describe('Web', () => {
    ...
    it('Update Akta', async () => {
        ...
        const data_in = Object.fromEntries(
            Object.entries(data.VALID_UPDATE_AKTA).map(([key, value]) => [key, typeof value == 'string' ? value.toLowerCase() : value])
          );
    ...
    it('Update Jadwal', async () => {
        const response = await QoinAPI.update_jadwal(data.VALID_UPDATE_JADWAL, token) //hit API
